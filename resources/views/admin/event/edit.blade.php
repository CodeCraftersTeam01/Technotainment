<x-admin.layout>
    <div class="w-full h-full bg-white shadow-lg rounded-3xl p-5">
        ...
        <form id="updateEventForm" action="{{ route('events.update', $event->event_id) }}" method="POST"
            enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Event Name -->
            <x-form.input label="Event Name" id="event_name" name="event_name" type="text" :default="$event->event_name" required />
            ...
            <x-form.file label="Event Logo" id="event_logo" name="event_logo" type="file" accept="image/*"
                ...
                @if ($event->event_logo)
                    <div class="mt-2 mb-3">
                        ...
                        <img src="{{ Storage::url($event->event_logo) }}" class="h-32 w-auto object-contain rounded-md"
            ...
            <!-- Event Theme -->
            <x-form.input label="Event Theme" id="event_theme" name="event_theme" type="text" :default="$event->event_theme" required />

            <!-- Event About -->
            <x-form.textarea label="Event About" id="event_about" name="event_about" :default="$event->event_about" required />

            <!-- Event Description -->
            <x-form.editor editorId="editor_event_description" label="Event Description" id="event_description"
                :default="$event->event_description" name="event_description" />

            <!-- Event Year -->
            <x-form.input label="Event Year" id="event_year" name="event_year" type="text" :default="$event->event_year"
                required />

            <!-- Event Status -->
            <x-form.select label="Event Status" id="event_status" name="event_status">
                <option value="nonactive"
                    {{ old('event_status', $event->event_status) == 'nonactive' ? 'selected' : '' }}>
                    Non-Active
                </option>
                <option value="active" {{ old('event_status', $event->event_status) == 'active' ? 'selected' : '' }}>
                    Active
                </option>
            </x-form.select>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('#updateEventForm').addEventListener('submit', function(e) {
                e.preventDefault();
