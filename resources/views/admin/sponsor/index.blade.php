<x-admin.layout header="Sponsors">
        ...
        <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">SPONSOR LOGO</th>
                    <th scope="col" class="px-6 py-4 font-medium text-gray-900">SPONSOR NAME</th>
            ...
            <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="font-medium text-gray-900">{{ $sponsor->sponsor_id }}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <img class="h-28 w-28 md:w-fit object-cover md:object-contain" src="{{ Storage::url($sponsor->sponsor_logo) }}"
                                alt="logo">
                        </td>
                        <td class="px-6 py-4">{{ $sponsor->sponsor_name }}</td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <x-button.action icon="components.icons.edit"
                                    href="{{ route('sponsors.edit', $sponsor->sponsor_id) }}"
                                    class="bg-indigo-50 text-indigo-700 ring-indigo-700/10 hover:bg-indigo-100">
                                    Edit
                                </x-button.action>
                                <form action="{{ route('sponsors.destroy', $sponsor->sponsor_id) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <x-button.action as="button" type="submit" icon="components.icons.delete"
                                        onclick="event.preventDefault(); alertDeleteModal({{ $sponsor->sponsor_id }});"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">No sponsor found</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 overflow-hidden">
            <div class="px-6 py-4 bg-indigo-600">
                <h3 class="text-lg font-medium text-white">Delete Sponsor</h3>
            </div>
            <div id="editTimelineForm" class="p-6">
                <p class="text-gray-500">Are you sure you want to delete this sponsor? This action cannot be undone.</p>
    ...
    <script>
        ...
        function alertDeleteModal(sponsorId) {
            document.getElementById('deleteModal').classList.remove('hidden');
            document.getElementById('deleteForm').action = "/sponsors/" + sponsorId;
        }
