<x-layout>
    ...
    <script>
        ...
        // Menampilkan nama file yang dipilih dan preview untuk abstract
        document.getElementById('work_abstract_input').addEventListener('change', function() {
            // Preview abstract text
            let fileAbstract = this.files[0];
            if (fileAbstract) {
                document.getElementById('work_abstract').textContent = fileAbstract.name;
            }
        });
            ...
            requiredFields.forEach(field => {
                ...
                if (field.isFile) {
                    ...
                    if (!element.value.trim()) {
                        ...
                        console.log(phoneRegex.test(element.value));
        ...
        `;
            ...
            for (let i = 1; i <= memberCount; i++) {
                const isOptional = (slug === '7lTI2n5EDK' && i === 2); // Anggota ke-5 hanya optional jika slug ini
            ...
            const abstractInput = document.querySelector('input[name="work_abstract"]');

            if (!abstractInput.files || abstractInput.files.length === 0) {
                hasErrors = true;
                const li = document.createElement('li');
                li.textContent = 'Abstract harus diisi';
                errorList.appendChild(li);
                abstractInput.parentElement.classList.add('border-red-500');
            } else {
                abstractInput.parentElement.classList.remove('border-red-500');
            }
