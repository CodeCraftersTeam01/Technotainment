<x-layout>
    ...
    <script>
        ...
        // Abstract specific
        const abstractInput = document.getElementById('work_abstract_input');
        if (abstractInput) {
            abstractInput.addEventListener('change', function() {
                const file = this.files[0];
                if (file) {
                    document.getElementById('work_abstract').textContent = file.name;
                    document.getElementById('work_abstract_preview').classList.remove('hidden');
                }
            });
        }
        ...
        if (nextBtn) {
            nextBtn.addEventListener('click', function() {
                ...
                // Add logo & invoice if not PES
                if (document.getElementById('team_logo')) {
                    requiredFields.push({ id: 'team_logo', name: 'Logo Team', isFile: true });
                }
                if (document.getElementById('team_invoice')) {
                    requiredFields.push({ id: 'team_invoice', name: 'Bukti Pembayaran', isFile: true });
                }
